...
Status: Stage 5 ✅ COMPLETE | Stage 6 ✅ COMPLETE | Stage 7 Phase A ✅ DONE | Phase B ✅ DONE | Phase C 🔄 IN PROGRESS (F8 ✅ F6 ✅ F7 📋)
...
- [x] **Phase C — Features bổ sung** 🔄 IN PROGRESS — F8 (Multiple CVs) ✅ + F6 (Profile Public View) ✅ + F7 (Job Comparison) 📋 pending. Session 13.
...
**Ưu tiên — Stage 7 Phase C: F7 Job Comparison (feature cuối cùng)**

F7 là frontend-only, không cần schema hay backend mới:
1. **`store/compareStore.ts`** — Zustand, persist localStorage, max 3 jobIds. Actions: `addJob`, `removeJob`, `clearJobs`, `isInCompare`.
2. **Nút "So sánh"** trên `JobCard.tsx` — toggle add/remove, disabled khi đã có 3 job + job này chưa trong list.
3. **`CompareBar.tsx`** — floating bar ở bottom khi `compareIds.length >= 1`: hiển thị tên job đã chọn, nút "Xem so sánh" → `/candidate/compare`, nút xóa từng job.
4. **`/candidate/compare/page.tsx`** — rewrite từ placeholder: bảng tối đa 3 cột, so sánh salary / location / jobType / workMode / match score % / skills (matched vs unmatched tags).
5. **tsc clean** cả frontend sau khi xong.

**Sau F7**: Stage 7 Phase C COMPLETE → commit → session wrap → chuẩn bị bảo vệ đồ án.
