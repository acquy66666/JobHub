# Project Plan: JobHub
Created: 2026-05-25
Last Updated: 2026-06-03 (session 12)
Current Stage: Stage 7
Status: Stage 5 ✅ COMPLETE | Stage 6 ✅ COMPLETE | Stage 7 Phase A ✅ DONE | Phase B ✅ DONE | Phase C 📋 PENDING
...
**Ưu tiên — Stage 7 Phase C (3 features còn lại):**
1. **F8 Multiple CVs**: Thêm model `CV` vào schema + db push. Backfill candidate có `cvUrl` → tạo CV row `isDefault=true`. Rewrite `/candidate/cv/page.tsx` (list + upload multi + set default + rename + delete). Sửa `ApplyModal.tsx` — dropdown chọn CV.
2. **F6 Profile Public View**: Thêm `publicSlug String? @unique` + `isPublicProfile Boolean @default(false)` vào `Candidate`. Backfill slug. API `GET /public/candidates/:slug` + `PATCH /candidate/profile/public-settings`. Trang `/u/[slug]` (public, no auth). Sửa `/candidate/preview/page.tsx` — embed + copy link. Toggle "Public" trong profile page.
3. **F7 Job Comparison**: `store/compareStore.ts` Zustand max 3 jobIds. Nút "So sánh" trên JobCard. Floating compare bar khi có 1+ job. Trang `/candidate/compare/page.tsx` — bảng 3 cột so sánh salary/location/type/mode/match%/skills.
