# Project Plan: JobHub
Created: 2026-05-25
Last Updated: 2026-06-03 (session 10)
Current Stage: Stage 7
Status: Stage 5 ✅ COMPLETE | Stage 6 ✅ COMPLETE | Stage 7 📋 PLANNED (chưa implement)
...
### 📋 Stage 7: Candidate Side Expansion — PLANNED (2026-06-03, session 10)
> Plan chi tiết đã write tại `C:\Users\Admin\.claude\plans\serene-knitting-octopus.md` và đã được user approve.
> Mục tiêu: thêm 8 features candidate + redesign bố cục theo nguyên tắc UX hiện đại (5–9 metrics/screen, information hierarchy, category grouping).
> Tham khảo competitive: LinkedIn, TopCV, VietnamWorks, Indeed, Glassdoor.

#### 8 Features sẽ thêm

- [ ] **F1 — In-app Notification Center** — Model `Notification` (NotificationType enum: APPLICATION_STATUS_CHANGED / NEW_JOB_FROM_FOLLOWED_COMPANY / NEW_MATCHED_JOB / JOB_EXPIRING_SOON / SYSTEM). Bell badge trên Navbar polling 60s + dropdown 5 noti gần nhất + page `/candidate/notifications` đầy đủ. API: `GET/PATCH/DELETE /notifications`, `GET /notifications/unread-count`, `PATCH /notifications/read-all`.
- [ ] **F2 — Followed Companies** — Model `FollowedCompany` (candidateId, employerId, @@unique). Nút "Theo dõi" trên `/companies/[id]`. Trang `/candidate/followed-companies`. Trigger noti khi admin approve job mới của employer được follow.
- [ ] **F3 — Recommended Jobs ("Việc làm phù hợp")** — Rule-based, không model mới. Logic: `score = 0.5*skillsMatch + 0.2*locationMatch + 0.2*industryFamiliarity + 0.1*recency`, bỏ job đã apply/save. `backend/src/services/recommendation.service.ts` + `GET /candidate/recommended-jobs?limit=10`. Page `/candidate/recommended` + section top 4 trên dashboard. Tái sử dụng `computeMatchScore` + `JobCard`.
- [ ] **F4 — Application Timeline + Kanban** — Model `ApplicationStatusHistory` (applicationId, fromStatus, toStatus, changedById, note). Hook vào `employer.service.updateApplicationStatus` tạo history + Notification. `GET /candidate/applications/:id/timeline`. Frontend: tab toggle List / Kanban (4 cột grid) / Timeline (vertical).
- [ ] **F5 — Recently Viewed Jobs** — Frontend-only, `lib/recentlyViewed.ts` localStorage max 20. Hook vào `/jobs/[id]` push khi mount. Section horizontal scroll trên dashboard. Page riêng `/candidate/recently-viewed`.
- [ ] **F6 — Profile Public View** — Thêm `publicSlug String? @unique` + `isPublicProfile Boolean @default(false)` vào Candidate. Migration backfill `nanoid(10)`. `GET /public/candidates/:slug` (no auth, ẩn email/phone). `PATCH /candidate/profile/public-settings`. Pages: `/u/[slug]` public + `/candidate/preview` embed + copy link.
- [ ] **F7 — Job Comparison** — Thay Saved Searches. `store/compareStore.ts` Zustand max 3 jobIds. Nút "So sánh" trên JobCard + saved-jobs. Floating compare bar. Page `/candidate/compare` — bảng 3 cột so sánh: salary, location, jobType, workMode, match%, skills matched/unmatched, benefits, experience.
- [ ] **F8 — Multiple CVs** — Model `CV` (candidateId, url, fileName, label, isDefault). Thêm `cvId String?` vào Application. Migration backfill: candidate cũ có `cvUrl` → tạo 1 CV `isDefault=true label="CV chính"`. Giữ tạm `Candidate.cvUrl`/`Application.cvUrl` cho backward-compat. API CRUD `/candidate/cvs` (+ default toggle). Apply Modal dropdown chọn CV.

#### Redesign Layout

- [ ] **Sidebar 4 nhóm, 11 items**: HOẠT ĐỘNG (Tổng quan / Đơn ứng tuyển / Việc đã lưu / Đã xem gần đây) · HỒ SƠ (Hồ sơ cá nhân / CV của tôi / Xem hồ sơ public) · KHÁM PHÁ (Việc làm phù hợp / So sánh việc làm / Công ty theo dõi) · CÀI ĐẶT (Thông báo việc làm). Notification page truy cập qua bell, không vào sidebar. Sửa `frontend/src/app/(candidate)/layout.tsx` tách `NAV_GROUPS`.
- [ ] **Dashboard mới**: HERO (Xin chào + Profile strength + Quick actions) → KEY METRICS 4 cards → GRID 2-col (Đề xuất cho bạn top 4 / Hoạt động gần đây mini feed) → Đơn gần đây (giữ) → Đã xem gần đây horizontal scroll. Rewrite `frontend/src/app/(candidate)/candidate/dashboard/page.tsx`.
- [ ] **Navbar Bell**: Component `NotificationBell.tsx`, badge unread, dropdown 5 noti + link "Xem tất cả". Chèn vào `Navbar.tsx` trước avatar dropdown. Mobile cũng có bell.

#### Roadmap 3 Phase

- [ ] **Phase A — Nền tảng + Redesign (sprint 1)**: F1 (Notification) + F5 (Recently Viewed) + Sidebar redesign + Dashboard redesign. Demo: layout mới + bell badge + dashboard rich hơn.
- [ ] **Phase B — Features cốt lõi (sprint 2)**: F3 (Recommended Jobs) + F2 (Followed Companies) + F4 (Timeline + Kanban). Demo: notification trigger từ 3 nguồn, recommendation engine, 3 view applications.
- [ ] **Phase C — Features bổ sung (sprint 3)**: F8 (Multiple CVs + migration) + F6 (Profile Public View) + F7 (Job Comparison). Demo cuối E2E: register → upload 2 CV → public on → follow X → view jobs → compare → apply → status change → noti → timeline → admin approve job mới của X → noti follow.
...
## Blockers
- Không còn blocker. TC4 đã verify ✅ (2026-06-02).

---

## Next Action (session tiếp theo)

**Ưu tiên — Bắt đầu Stage 7 Phase A:**
1. Kiểm tra `prisma migrate status` để hiểu drift hiện tại trước khi tạo migration mới
2. Thêm model `Notification` + enum `NotificationType` vào schema.prisma
3. Build backend Notification service + routes + controllers
4. Build frontend `NotificationBell.tsx` + page `/candidate/notifications`
5. Implement `lib/recentlyViewed.ts` (localStorage) + hook vào `/jobs/[id]`
6. Redesign Sidebar: tách `NAV_GROUPS` 4 nhóm trong `(candidate)/layout.tsx`
7. Redesign Dashboard layout với HERO + grid 2-col + sections mới
