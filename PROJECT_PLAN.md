...
Status: Stage 5 ✅ COMPLETE | Stage 6 Nhóm 1 ✅ DONE | Stage 6 Nhóm 2 ✅ DONE (core 4/6 features)
...
**Ưu tiên cao — Deploy & verify features mới:**
1. Push code → Vercel + Render tự redeploy
2. Test `/admin/dashboard` → xem LineChart "Xu hướng 8 tuần"
3. Test `/jobs` (login candidate) → xem badge `% phù hợp` trên JobCard
4. Test `/jobs/:id` → xem sidebar card "Độ phù hợp với bạn" (matched/unmatched skills)
5. Test `/employer/candidates` → search theo skill + location

**Nhóm 2 còn lại (optional, nếu còn thời gian):**
- **Bulk Export Applications** (~2–3h) — CSV stream endpoint, không cần schema mới. Nhanh nhất.
- **Content Moderation Queue** (~4–5h) — model `Report` mới, cần `db push`.

**Nhóm 3 (chỉ làm nếu muốn điểm kỹ thuật thêm):**
- Email Job Alerts, Audit Logs, Fraud Detection — đều high complexity, không bắt buộc cho đồ án.

**Lưu ý kỹ thuật email (quan trọng):**
- Email dùng Brevo REST API (HTTPS port 443) — KHÔNG dùng SMTP (bị block trên Render)
- Env vars trong Render: `BREVO_API_KEY` + `BREVO_SENDER_EMAIL` (email đã verify trong Brevo)
