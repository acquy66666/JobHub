...
Status: Stage 5 ✅ COMPLETE | Stage 6 ✅ COMPLETE | Stage 7 ✅ COMPLETE (Phase A ✅ Phase B ✅ Phase C ✅)
...
- [ ] **E2 — Job Templates** — Model `JobTemplate` (employerId, title, description, requirements, benefits, jobType, workMode, industry). CRUD `/employer/job-templates`. Dropdown "Dùng mẫu" ở bước tạo tin mới + "Lưu làm mẫu" ở bước review.
- [ ] **E3 — Ghi chú nội bộ** — Model `ApplicationNote` (applicationId, authorId, content, createdAt). Hiển thị trong accordion ứng viên, chỉ employer thấy, không lộ phía candidate.
- [ ] **E7 — Tag ứng viên** — Mở rộng/bổ sung tag tùy chỉnh (free-text hoặc enum mở rộng). Dropdown tag trên từng application card, filter by tag trong danh sách ứng viên.
- [ ] **E6 — Screening Questions** — Employer cấu hình câu hỏi Yes/No hoặc text cho từng JD. ApplyModal hiển thị và bắt buộc điền. Model `ScreeningQuestion` + `ScreeningAnswer`. Cần regression test kỹ ApplyModal sau khi sửa.
- [ ] **E9 — Lập lịch phỏng vấn** — Model `InterviewSchedule` (applicationId, scheduledAt, location, meetingLink, status PENDING/CONFIRMED/CANCELLED). Employer tạo slot → email tự động mời candidate → candidate xác nhận. Full loop: DB + Email + UI.
...
**Stage 8 Sprint 1 COMPLETE (session 19). Sprint 2 tiếp theo.**

Ưu tiên ngay:
1. **E2 — Job Templates** — model `JobTemplate` đã có trong schema từ Stage 6, chỉ cần UI: dropdown "Dùng mẫu" ở new-job form + "Lưu làm mẫu" ở step review.
2. **E3 — Ghi chú nội bộ** — model `ApplicationNote` (applicationId, authorId, content), endpoint GET/POST, hiện trong accordion ứng viên.
3. **E7 — Tag ứng viên mở rộng** — bổ sung free-text tags hoặc mở rộng ApplicationTag enum, filter by tag trong danh sách.

**Lưu ý kỹ thuật Sprint 1:**
- `backend/src/generated/prisma/` trong .gitignore — `prisma generate` tự chạy khi Render build, schema.prisma đã có `NEW_APPLICATION`.
- Kanban fetch với `limit: 200`, queryKey riêng `[..., 'kanban']` để không conflict với list view.
- Email `sendApplicationStatusEmail` signature đổi: thêm param `note?: string` — backward compatible (optional).
