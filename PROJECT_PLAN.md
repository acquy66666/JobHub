# Project Plan: JobHub
Created: 2026-05-25
Last Updated: 2026-06-04 (session 20)
Current Stage: Stage 8
Status: Stage 5 ✅ COMPLETE | Stage 6 ✅ COMPLETE | Stage 7 ✅ COMPLETE | Stage 8 Sprint 1 ✅ Sprint 2 partial (E2✅ E3✅ E7 pending)
...
**Stage 8 Sprint 2 gần xong (session 20). E2 ✅ (đã có sẵn) + E3 ✅ (commit cc149bb, QA PASS). Chỉ còn E7.**

Ưu tiên ngay:
1. **E7 — Tag ứng viên mở rộng** — bổ sung filter by tag trong applications list (SHORTLISTED/POTENTIAL/ON_HOLD đã có). Có thể thêm tag type mới hoặc chỉ cần filter UI rõ hơn. Backend: không cần schema mới, chỉ cần query filter param `?tag=` trong GET /applications.
2. **Sprint 3 tiếp theo:** E6 Screening Questions → E9 Interview Scheduler → E10 Salary Benchmark.

**Lưu ý kỹ thuật Sprint 1+2:**
...
- `ApplicationNote` model đã có trong DB (Supabase migration applied session 20). Relation: `Application.internalNotes[]` + `User.applicationNotes[]`.
