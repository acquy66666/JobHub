# Project Plan: JobHub
Created: 2026-05-25
Last Updated: 2026-06-04 (session 21)
Current Stage: Stage 8
Status: Stage 5 ✅ COMPLETE | Stage 6 ✅ COMPLETE | Stage 7 ✅ COMPLETE | Stage 8 Sprint 1 ✅ Sprint 2 ✅ (E2✅ E3✅ E7✅) Sprint 3 partial (E6✅ E9 E10 pending)
...
**Stage 8 Sprint 2 HOÀN THÀNH (session 21). E7 ✅ + E6 ✅. Production deployed.**

Ưu tiên ngay:
1. **E9 — Lập lịch phỏng vấn** — Model `InterviewSchedule` (applicationId, scheduledAt, location, meetingLink, status PENDING/CONFIRMED/CANCELLED). Employer tạo slot → email tự động mời candidate → candidate xác nhận. Full loop: DB + Email + UI. Đây là feature phức tạp nhất còn lại, phù hợp để demo cho hội đồng.
2. **E10 — Salary Benchmark** — Aggregate MIN/MAX/AVG từ `Job.salaryMin/salaryMax` group by title + industry. Widget bên cạnh form đăng tin. Không cần schema mới, cần seed data đủ đa dạng salary.

**Lưu ý kỹ thuật Sprint 1+2+3:**
...
- `ApplicationNote` model đã có trong DB (Supabase migration applied session 20).
- `ScreeningQuestion` + `ScreeningAnswer` đã có trong DB (Supabase migration applied session 21). ApplyModal nhận `screeningQuestions` prop optional → không break /candidate/compare page.
- E7 list query key: `[...queryKeys.employerJobApplications(jobId, page), filterStatus, filterTag]` — invalidate bằng prefix `queryKeys.employerJobApplications(jobId)` vẫn work.
