...
- [ ] TC2b: Login employer `employer@jobhub.vn` / `Demo@2026` → `/employer/dashboard` (code reviewed ✓, ready to test)
- [ ] TC2c: Login admin `admin@jobhub.vn` / `Demo@2026` → `/admin/dashboard` (code reviewed ✓, ready to test)
...
**Ưu tiên 1 — Push code lên GitHub để trigger Vercel redeploy** (BUG-4 và BUG-5 fix chưa được deploy):
```bash
git push origin main
```

**Ưu tiên 2 — Verify TC2b → TC7 sau khi deploy xong:**
- TC2b: Login employer `employer@jobhub.vn` → phải thấy Navbar đầy đủ + sidebar "TechCorp Vietnam"
- TC2c: Login admin `admin@jobhub.vn` → phải thấy Navbar đầy đủ + sidebar "Admin Panel"
- TC3: Apply job end-to-end (login candidate → /jobs → chọn job → Ứng tuyển → submit)
- TC5: Upload CV PDF `/candidate/cv` (cần Cloudinary hoạt động)
- TC6: Admin duyệt PENDING job `/admin/jobs`
- TC4: Email notification (employer đổi status → candidate nhận email `hunguu05@gmail.com`)
- TC7: Mobile 375px — Navbar hamburger + sidebar slide-in trên cả 3 dashboards
