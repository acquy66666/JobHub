...
**Ưu tiên — Verify production + Chuẩn bị bảo vệ:**
1. Verify `/admin/logs` — 19 bản ghi hiển thị, filter theo action hoạt động
2. Verify `/candidate/job-alerts` — 3 alerts hiển thị, toggle/delete/tạo mới hoạt động
3. Verify `/admin/jobs` tab "⚠ Gắn cờ" — 3 jobs flagged hiển thị badge ⚠
4. Verify `/admin/reports` — 8 báo cáo (4 PENDING chờ xử lý), review/dismiss hoạt động
