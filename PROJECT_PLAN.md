# Project Plan: JobHub
Created: 2026-05-25
Last Updated: 2026-06-04 (session 23 — UI/UX audit)
...
**Session 23 (2026-06-04) — UI/UX Audit done. Đặt UI/UX Optimization Sprint làm ƯU TIÊN CAO NHẤT trước E10.**
...
- [ ] **QW-1 — Mobile header label động** — Thay `"Dashboard"` cứng trong [(candidate)/layout.tsx:133](frontend/src/app/(candidate)/layout.tsx#L133) và [(employer)/layout.tsx:98](frontend/src/app/(employer)/layout.tsx#L98) bằng logic lấy label từ NAV theo `pathname` hiện tại.
- [ ] **QW-2 — Employer Dashboard skeleton + fix limit bug** — Thêm `animate-pulse` loading cho 4 stat cards (copy pattern từ Admin Dashboard). Đồng thời fix `api.get("/employer/jobs", { params: { limit: 100 } })` ở [(employer)/employer/dashboard/page.tsx:12](frontend/src/app/(employer)/employer/dashboard/page.tsx#L12) — chuyển sang endpoint `/employer/job-stats` (đã có sẵn từ E1) để stats chính xác.
- [ ] **QW-3 — staleTime cho query thường refetch** — `staleTime: 30_000` cho `candidateApplications`, `employerJobApplications`, `notifications`. Giảm flicker skeleton khi switch tab.
- [ ] **QW-4 — focus-visible ring global** — Thêm `focus-visible:ring-2 focus-visible:ring-[#7C3AED]/50 focus-visible:outline-none` vào button/link patterns. Audit `components/jobs/JobCard.tsx`, `CompareBar.tsx`, `NotificationBell.tsx`, layout sidebars. Accessibility cơ bản.
- [ ] **QW-5 — Empty state `/candidate/compare`** — Thêm block hướng dẫn (icon ⚖, heading "Chưa có việc làm để so sánh", text giải thích cách thêm qua nút ⚖ trên JobCard) tại [(candidate)/candidate/compare/page.tsx](frontend/src/app/(candidate)/candidate/compare/page.tsx) khi `compareJobs.length === 0`.
