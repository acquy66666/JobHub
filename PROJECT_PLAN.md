...
#### Nhóm 1 — Low Complexity (Quick Wins, ~1–3 ngày/feature)

- [ ] **Employer Verification Badge** — Admin toggle `isVerified` trên trang admin/users → badge "Đã xác thực" hiển thị trên company page. (`isVerified` field đã có trong Employer schema, chỉ cần UI.)
- [ ] **Profile Completeness Meter** — Progress bar trên candidate dashboard tính % các trường đã điền (fullName, phone, headline, skills, cvUrl, experience, education). Pure frontend, compute từ profile data.
- [ ] **Job Analytics Columns** — Thêm `viewCount` vào bảng employer/jobs (đã có trong DB). Tính conversion rate = applications / views.
- [ ] **Job Post Templates** — Employer lưu template mô tả công việc tái sử dụng. Cần model `JobTemplate` mới + UI tạo/chọn template khi đăng tin.
...
- [ ] **Candidate Shortlist & Tags** — Employer tag ứng viên là Shortlisted / On Hold (màu sắc khác nhau). Extend `Application` model với `tag` field. UI: dropdown tag bên cạnh status selector.
...
**Ưu tiên 1 — Verify các TC còn lại trên deployed site:**
- TC6: Admin duyệt PENDING job `/admin/jobs` (BUG-8 đã fix, optimistic update, cần verify)
- TC7: Mobile 375px — Navbar hamburger + sidebar slide-in (Chrome DevTools 375px)
- TC4: Email notification — employer đổi trạng thái đơn → `hunguu05@gmail.com` nhận email

**Ưu tiên 2 — Stage 6 Nhóm 1 (Quick Wins, bắt đầu khi Stage 5 verify xong):**
- Employer Verification Badge (2h — toggle isVerified + badge UI)
- Profile Completeness Meter (1 ngày — pure frontend)
- Job Analytics Columns (0.5 ngày — thêm viewCount + conversion rate vào employer/jobs)
- Job Post Templates (1–2 ngày — model mới + UI)
