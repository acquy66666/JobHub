...
Status: Stage 5 ✅ COMPLETE | Stage 6 Nhóm 1 ✅ DONE | Stage 6 Nhóm 2 ✅ DONE (6/6 features)
...
**Ưu tiên cao — Deploy & verify Stage 6 features:**
1. Push code → Vercel + Render tự redeploy (2 commits mới session này)
2. Test `/jobs/:id` (login candidate) → nút "Báo cáo vi phạm" + modal hoạt động
3. Test `/admin/reports` → queue hiển thị, nút Đã xem xét / Bỏ qua hoạt động
4. Test employer `/jobs/:id/applications` → nút "Xuất CSV" → file tải về đúng
5. Verify lại các features Nhóm 2 cũ: LineChart, Match Score, Candidate Search

**Nhóm 3 (nếu muốn điểm kỹ thuật thêm — không bắt buộc):**
- **Email Job Alerts** — Candidate opt-in + cron job gửi email hàng ngày (high complexity)
- **Activity / Audit Logs** — Ghi lại mọi action admin vào `AuditLog` table
- **Fraud / Spam Detection** — Rule-based auto-flag jobs vi phạm
