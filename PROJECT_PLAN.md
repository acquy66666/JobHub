...
Các bước còn lại trước bảo vệ:
1. **Deploy production** — Push lên GitHub → Render auto-deploy backend, Vercel auto-deploy frontend. Nếu Render không tự deploy: vào dashboard → "Manual Deploy".
2. **Verify production** — Kiểm tra F7 (compare bar), F6 (public profile /u/slug), F8 (CV list) hoạt động trên production URL.
3. **TC7 Mobile responsive 375px** — Hoãn từ Stage 5, nên test trước bảo vệ.
4. **Demo script** — Chuẩn bị luồng demo: login candidate → xem job → so sánh → nộp đơn → xem timeline; login employer → quản lý ứng tuyển; login admin → duyệt job + xem logs.
5. **Slide thuyết trình** — Highlight: JWT refresh rotation, RBAC 3 role, Cloudinary upload, Brevo email, 8 Stage 7 features, dark theme design system.
