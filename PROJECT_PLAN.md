...
## Blockers
- TC4 email chưa verify: cần kiểm tra GMAIL_USER + GMAIL_APP_PASSWORD trong Render Environment sau khi redeploy `bffd5fd`.

---

## Next Action (session tiếp theo)

**Ưu tiên 1 — Verify TC4 email sau khi Render redeploy `bffd5fd`:**
- Đăng ký tài khoản mới với email thật (`hunguu05@gmail.com`) tại `/register`
- Xác nhận nhận được OTP email → GMAIL credentials hoạt động
- Employer đổi trạng thái đơn → xác nhận candidate nhận email thông báo
- Nếu vẫn không nhận: kiểm tra `GMAIL_USER` + `GMAIL_APP_PASSWORD` trong Render Environment

**Ưu tiên 2 — Stage 6 Nhóm 2 (khi TC4 verify xong):**
- Platform Analytics Charts v2 (~2 ngày — time-series LineChart cho admin dashboard, không cần schema mới)
- Candidate Shortlist & Tags (~2 ngày — extend Application model với `tag` field, dropdown UI)
- Job Match Score (~3 ngày — string matching skills, endpoint GET /api/jobs/:id/match-score, badge trên JobCard)
