# Project Plan: JobHub
Created: 2026-05-25
Last Updated: 2026-06-03 (session 11)
Current Stage: Stage 7
Status: Stage 5 ✅ COMPLETE | Stage 6 ✅ COMPLETE | Stage 7 Phase A ✅ DONE | Phase B/C 📋 PENDING
...
### 🔄 Stage 7: Candidate Side Expansion — IN PROGRESS (2026-06-03)
> Plan chi tiết tại `C:\Users\Admin\.claude\plans\serene-knitting-octopus.md` (đã được user approve).
> Phase A ✅ DONE (session 11, commit `12d7f01`). Phase B + C pending.
...
- [ ] **F2 — Followed Companies** — Model `FollowedCompany`. Nút "Theo dõi" trên `/companies/[id]`. Trang `/candidate/followed-companies` (hiện là placeholder). Trigger noti khi admin approve job mới.
- [ ] **F3 — Recommended Jobs** — `recommendation.service.ts` rule-based (score formula). `GET /candidate/recommended-jobs?limit=10`. Page `/candidate/recommended` (hiện placeholder) + section trên dashboard.
- [ ] **F4 — Application Timeline + Kanban** — Model `ApplicationStatusHistory`. Hook `employer.service.updateApplicationStatus`. Tab toggle List/Kanban/Timeline trong `/candidate/applications`.
...
- [ ] **F6 — Profile Public View** — `publicSlug` + `isPublicProfile` vào Candidate. `/u/[slug]` public page. `/candidate/preview` (hiện placeholder).
- [ ] **F7 — Job Comparison** — `store/compareStore.ts` Zustand max 3. Floating bar. `/candidate/compare` page (hiện placeholder).
- [ ] **F8 — Multiple CVs** — Model `CV`. Backfill candidate cũ. API CRUD. Apply Modal dropdown.
...
- [ ] **Phase B — Features cốt lõi** — F3 (Recommended Jobs) + F2 (Followed Companies + trigger noti) + F4 (Timeline + Kanban view).
- [ ] **Phase C — Features bổ sung** — F8 (Multiple CVs) + F6 (Profile Public View) + F7 (Job Comparison).
