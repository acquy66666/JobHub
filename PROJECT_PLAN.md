...
- [ ] TC2b: Login employer `employer@jobhub.vn` / `Demo@2026` → `/employer/dashboard`
- [ ] TC2c: Login admin `admin@jobhub.vn` / `Demo@2026` → `/admin/dashboard`
- [ ] TC3: Luồng apply job end-to-end (login candidate → /jobs → chọn job → Ứng tuyển → submit)
- [ ] TC4: Email notification — employer đổi trạng thái đơn → candidate nhận email tại `hunguu05@gmail.com`
- [ ] TC5: Upload CV PDF — `/candidate/cv` → upload file PDF ≤5MB → URL Cloudinary trả về
- [ ] TC6: Admin duyệt PENDING job — `/admin/jobs` → tab PENDING → Duyệt → job chuyển ACTIVE
- [ ] TC7: Mobile responsive 375px — Navbar hamburger không overflow

#### Bugs phát hiện khi verify — cần fix trước khi bảo vệ
- [ ] **BUG-1: Navbar trống sau khi đăng nhập** — header không hiển thị tên user / avatar / dropdown. Kiểm tra `frontend/src/components/layout/Navbar.tsx` — có thể do Zustand store không persist qua SSR hoặc component không re-render sau `setAuth`.
- [ ] **BUG-2: Không có nút đăng xuất** — cần có logout button trong dropdown menu của Navbar. Gọi `POST /api/auth/logout` + `clearAuth()` + redirect `/login`.
- [ ] **BUG-3: /jobs và /companies thiếu link về trang chủ** — thêm breadcrumb hoặc link "← Trang chủ" vào header/layout của 2 trang này.
...
**Ưu tiên 1 — Fix 3 UI bugs** (Claude làm):
- BUG-1 + BUG-2: Navbar hiển thị user info + nút logout sau khi đăng nhập
- BUG-3: Thêm link về trang chủ trên /jobs và /companies

**Ưu tiên 2 — Hoàn thành TC2b → TC7** (bạn tự verify từng bước):
- TC2b: Login employer → `/employer/dashboard`
- TC2c: Login admin → `/admin/dashboard`
- TC3: Apply job end-to-end
- TC4: Email notification (employer đổi trạng thái → candidate nhận email)
- TC5: Upload CV PDF → Cloudinary
- TC6: Admin duyệt PENDING job
- TC7: Mobile responsive 375px
