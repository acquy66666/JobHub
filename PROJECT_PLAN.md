...
Status: Stage 5 ✅ COMPLETE | Stage 6 Nhóm 2 In Progress
...
**Stage 6 Nhóm 2 — chọn 1 trong 3 feature:**
- **Platform Analytics Charts v2** (~2–3h) — time-series LineChart cho admin dashboard, `$queryRaw` GROUP BY week, không cần schema mới. Quick win nhất.
- **Job Match Score** (~4–5h) — string matching skills candidate vs job requirements, endpoint `GET /api/jobs/:id/match-score`, badge % trên JobCard + job detail page.
- **Candidate Database Search** (~4–6h) — employer tìm candidate theo skill/location/headline, endpoint `GET /api/employers/candidates/search`.

**Lưu ý kỹ thuật email (quan trọng):**
- Email dùng Brevo REST API (HTTPS port 443) — KHÔNG dùng SMTP (bị block trên Render)
- Env vars trong Render: `BREVO_API_KEY` (key `xkeysib-...`) + `BREVO_SENDER_EMAIL` (email đã verify)
- Sender email phải được verify trong Brevo → Settings → Senders & IPs
