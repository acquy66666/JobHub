...
**Session 23 (2026-06-04) — UI/UX Audit + QW-1:** Subagent review toàn frontend. Phát hiện 10 vấn đề + 5 quick wins + 5 improvements lớn. Đặt UI/UX Sprint làm ưu tiên cao nhất trước E10. **QW-1 ✅** — Commit `eb5f154`. Mobile header label động: 3 layout (candidate/employer/admin) tra cứu label từ NAV theo `pathname` (exact match + startsWith fallback, candidate sort by length cho sub-route động). Tsc clean. **Còn pending:** QW-2 (Employer Dashboard skeleton + fix `limit:100` bug → dùng `/employer/job-stats` đã có sẵn từ E1), QW-3 (staleTime cho query), QW-4 (focus-visible ring), QW-5 (empty state compare), IMP-1..5, rồi E10.
